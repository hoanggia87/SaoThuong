  ...
  return (
    <div className="min-h-full flex flex-col p-4 sm:p-10 overflow-y-auto">
      <div className="flex items-center gap-2 mb-4 sm:mb-6">
        ...
        <h1 className="text-xl sm:text-4xl font-display font-extrabold flex-1 truncate">{dict.rewards.title}</h1>
        <div className="bg-white rounded-full px-3 py-1.5 sm:px-5 sm:py-2 shadow-toy-sm shrink-0">
          <span className="text-lg sm:text-2xl font-display font-extrabold text-candy-yellow whitespace-nowrap">
            {kid.totalStars} ⭐
          </span>
        </div>
      </div>
      ...
      {sorted.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5 max-w-5xl mx-auto w-full">
          {sorted.map((r, i) => {
            ...
            return (
              <motion.button
                ...
                disabled={!can}
                className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-toy text-left transition-all ${
                  can
                    ? 'bg-gradient-to-br from-white to-pink-50 hover:shadow-2xl'
                    : 'bg-slate-100 opacity-70'
                }`}
              >
                <div className="text-5xl sm:text-7xl mb-2 sm:mb-3">{r.icon}</div>
                <div className="text-base sm:text-2xl font-display font-bold mb-1.5 sm:mb-2 line-clamp-2">{r.name}</div>
                <div
                  className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 sm:px-4 sm:py-2 text-sm sm:text-base font-bold ${
      ...
      <AnimatePresence>
        {confirming && (
          <motion.div
            ...
            className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur p-4 sm:p-6"
          >
            <motion.div
            ...
            >
              <div className="text-5xl sm:text-7xl mb-2 sm:mb-3">{confirming.icon}</div>
              <div className="text-xl sm:text-2xl font-display font-bold mb-2">
                {t(dict.rewards.redeemQuestion, { name: confirming.name })}
              </div>
              <div className="text-sm sm:text-base text-slate-500 mb-5 sm:mb-6">
        ...
        {success && (
          ...
          >
            ...
            >
              <div className="text-6xl sm:text-8xl mb-3 sm:mb-4 animate-pop">{success.icon}</div>
              <div className="text-2xl sm:text-3xl font-display font-extrabold mb-2 text-candy-purple">
                {dict.rewards.congrats}
              </div>
              <div className="text-base sm:text-xl mb-5 sm:mb-6">
